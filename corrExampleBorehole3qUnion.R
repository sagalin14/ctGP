...
if(.Platform$OS.type == "windows"){
  ...
  pathlink = "/Users/jialingzheng/Downloads/SUMO"
}
...
for(num in numList){
  ...
  for(comm in c(20)){
    ...
    if(comm == (num+5)){type <- "Comm"}else{type <- "Union"}
    
    commonPts = comm
    indepPts = num - commonPts + 5
    ...
    set.seed(99999)
    if(pts == 0){
      DesignList <- lapply(1:iternum, FUN = function(x) return(NULL))
    }else{
      DesignList <- lapply(1:iternum, FUN = function(x) return(maximinLHS(pts, 5, maxIter = 10000000)))
    }
    ...
    for(iter in 1:iternum)
    {
      ...
      for(case in 1:levelNum){
        x.tmp <- rbind(DesignList[[iter]][sample(nrow(DesignList[[iter]]), comm), ])
      ...
      diff <-
        QQGP.ori.prediction(QQGP.model = QQGPfit,
                            newPoint = coerceFun.ori(data = data.test, design = data.test[, 1:5]))$predictValue - data.test[, dim(data.test)[2]]
      mse <- sum(diff ^ 2) / dim(data.test)[1]
      QQGPpredictTime2 <- proc.time()
      QQGPperform <-
        c(
          mse = sqrt(mse),
          CPUtime = (QQGPTime2 - QQGPTime1)[3],
          PredictTime = (QQGPpredictTime2 - QQGPpredictTime1)[3]
        )
      ...
      print(paste0(
        ...
        round(sqrt(mse), 2),
        ", time: ",
        round((QQGPTime2 - QQGPTime1)[3]),
        ", predtict Time: ",
        round((QQGPpredictTime2 - QQGPpredictTime1)[3])
      ))
